 ...
 */
 	...
 	var imgArray = $$("#fruits > img");
	for(var i=0; i<imgArray.length; i++) {
		new Draggable(imgArray[i], {revert: true});
	}
	Droppables.add("basket", {onDrop: fruitSelect});
	Droppables.add("fruits", {onDrop: fruitUnselect});
});
		...
		if(drop.id == "basket") {
			...
			if(imgArray.length<3) {
				...
				if(imgArray.length >= 1) {
					$("basket").removeClassName(imgArray[imgArray.length-1].getAttribute("alt"));
				}
				$("basket").addClassName(drag.getAttribute("alt"));
			}
		}
	}
}
	...
	if(drag.parentNode.id != drop.id) {
		if(drop.id == "fruits") {
			...
			for(var i=0; i<liArray.length; i++) {
				$("basket").removeClassName(liArray[i].innerHTML);
